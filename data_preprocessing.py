...
def multi_plots(data, col_names=None):
    col = data.shape[1]
    fig, axs = plt.subplots(col, 1, figsize=(10, 2.5 * col))
    ax_lst = axs.flat if col > 1 else [axs]
    ...
    for i in range(col):
        ...
        ax.plot(feature)
        if col_names:
            ax.set_title(f'{col_names[i]}', fontsize=10, fontweight='bold')
...
c_names = ["temperature", "wind speed","mean sea level pressure", "surface solar radiation", "surface thermal radiation", "total cloud cover"]
multi_plots(X, col_names =  c_names)
multi_plots(final_X, col_names = c_names)
# print(X[:, [0, 2,3,4]])
